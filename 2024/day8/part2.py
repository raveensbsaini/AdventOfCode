...
def count_location(pair,antinode_loc,matrix):
    x = pair[0]
    y = pair[1]
    if tuple(x) not in antinode_loc:
        antinode_loc[tuple(x)] = True
    if tuple(y) not in antinode_loc:
        antinode_loc[tuple(y)] = True
    row_diff  = (x[0] -y[0])
    col_diff = (x[1] - y[1])
    ...
    while True:
        if 0 <= location_1[0] < len(matrix) and 0 <= location_1[1] <len(matrix[0]) :
            ...
            location_1 = [location_1[0] - row_diff,location_1[1]-col_diff]
        else:
            break
    while True:
        if 0 <= location_2[0] < len(matrix) and 0 <= location_2[1] <len(matrix[0]) :
            antinode_loc[tuple(location_2)] = True
            location_2 = [ row_diff+location_2[0] , col_diff+location_2[1] ]
        else:
            break
